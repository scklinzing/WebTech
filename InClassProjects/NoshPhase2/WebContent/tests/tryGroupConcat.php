...
echo "<h1>Testing justing left join to get rows with the tags</h1>";
$comments = array();
try {
	$db = Database::getDB ();
	$query = "SELECT comments.commentId, evaluationUrl, comment, commentTagMap.commentTagId  FROM comments LEFT JOIN commentTagMap
...
function getCommentArray($rowSets) {
	$comments = array ();
	foreach ( $rowSets as $commentRow ) {
		$tags = $commentRow['taglist'];
		echo "<h2>tags are: $tags</h2>";
